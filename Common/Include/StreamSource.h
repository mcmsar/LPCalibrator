...
//! A data stream source.  Data will flow be written into this class from external data
//! sources.  This data will then be read from the source by its containing channel.
class CEMSStreamSource : public IEMSSeqStream, public CEMSLogClient, public CApiObjBase
{
	public:
		CEMSStreamSource();
		CEMSStreamSource( const CEMSStreamSource& x );
		virtual ~CEMSStreamSource();
		...
		//! IEMSSeqStream method.  Fill the source.
		...
		//! Determine whether the source may still be holding unwritten data.
		bool HasData();

		//! Mark the source as being in use or not.  If not in use, it will be removed from the
		//! channel's managed set of sources during cleanup.
		void SetInUse( const bool cbInUse = true );

		//! Determine whether a source is being used or not.
		bool GetInUse();
	...
	private:
		...
		bool			m_bMayHoldData;
		bool			m_bInUse;
		CEMSCriticalSection	m_csReadWrite;
