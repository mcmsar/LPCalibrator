	...
	try
	{
		...
		if( !setSurvivors.empty() || !setToDrop.empty() )
		{
			...
			while( pObj = m_lstCrossChannelBuffer.GetNext() )
			{
				...
				{
					_RecordDispatch( pObj->GetLutId(), pObj->GetSatId(), pObj->GetBeaconID(), pObj->GetTimeMsg().intTime, pObj->GetFrequency() );
...
void
CHGTSPCalibrateEngine::_RecordDispatch( ULONG ulLutId, ULONG ulSatId, INT64 i64BcnId, INT64 i64ReceiveTimeNanos, double dFrequency )
{
...
bool
CHGTSPCalibrateEngine::_IsLateArrivalDuplicate( ULONG ulLutId, ULONG ulSatId, INT64 i64BcnId, INT64 i64ReceiveTimeNanos, double dFrequency )
{
