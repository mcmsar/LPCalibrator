...
BOOL CPrinter::StartPrinting()
    {
	 ...
	 std::basic_string<TCHAR> otszName;
     
	 if( m_wszDocName.empty() )
	 {
	 ...
	 else
	 {
		 ...
		 otszName = m_wszDocName.c_str();
#else
		 otszName = CEMSConversionUtil::ConvertToString( m_wszDocName.c_str() ).c_str();
#endif
	 }

	 info.lpszDocName = otszName.c_str();
...
void CPrinter::SetDocumentName( const wchar_t* cwszName )
{
	if( cwszName )
	{
		m_wszDocName = cwszName;
	}
}